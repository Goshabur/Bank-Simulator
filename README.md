...
Начиная с этого задания и дальше расставляйте `noexcept` на функциях, которые не могут кидать исключения.

При запуске под Valgrind решение компилируется с флагом `-DSMALL_TESTS`, чтобы тесты стали чуть поменьше.
При запуске под Address Sanitizer под Linux (но не под macOS) решение компилируется с флагом `-DLESS_USERS_TEST`,
...
## Сроки сдачи
Задание выдано 7 февраля 2022 (понедельник),
Ниже в каждом случае указано московское время.

* **Дедлайн сдачи:** 17 февраля (четверг), **22**:59.
* Ожидаемый срок проверки: 24 февраля (четверг).
...
* **Срок исправлений**: 3 марта (четверг), **22**:59.
